...
public class SemanticChecker implements ASTVisitor {
    ...
    @Override public void visit(funcDef it) {
        ...
        returndone = false;
        current = new Scope(current);
        it.List.forEach(x -> current.defineVariable(x.id, new varType(global.getType(x.type), x.id), x.pos));
        it.body.accept(this);
        current = current.parentScope;
        if (it.id.equals("main")) returndone = true;
